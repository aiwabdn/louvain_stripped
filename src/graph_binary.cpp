...
Graph::Graph(vector<vector<pair<int, long double> > > &collected_links) {
    nb_nodes = collected_links.size();
    degrees.resize(nb_nodes);
    degrees[0] = collected_links[0].size();
    for(int i=1; i<nb_nodes; i++) {
        degrees[i] = degrees[i-1] + collected_links[i].size();
    }
    nb_links = degrees[nb_nodes-1];
    total_weight = 0.0L;
    for(int i=0; i<nb_nodes; i++) {
        for(unsigned int j=0; j<collected_links[i].size(); j++) {
            links.push_back(collected_links[i][j].first);
            weights.push_back(collected_links[i][j].second);
        }
        //collected_links.erase(collected_links.begin());
    }
    //delete &collected_links;
    for (int i=0 ; i<nb_nodes ; i++) {
